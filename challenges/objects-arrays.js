...
uniInName(graduates);
unisWithUni.forEach(unisWithUni => console.log(unisWithUni));
...
function lowPopFilter(array) {
    for (let i = 0; i < array.length; i++) {
        if (array[i].population < 5) {
            lowPopulationAnimals.push(array[i]);
        }
    }
}
lowPopFilter(zooAnimals);
